...
```sh
bun --filter @packages/backend setup
```
...
Create `.env.local` files in `apps/web` from the provided `.example.env` files.
...
```sh
cd ../../packages/backend && bun exec convex deploy --cmd 'cd ../../apps/web && bun build' --cmd-url-env-var-name NEXT_PUBLIC_CONVEX_URL
```
...
```sh
bun --filter web-app add mypackage@latest
bun --filter @packages/backend add mypackage@latest
```
